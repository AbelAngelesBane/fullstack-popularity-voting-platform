...
const ProfileScreen = () => {
  ...
  return (
    <View className='p-4 bg-background flex flex-1 '>
      <Pressable className='items-end p-2' onPress={() => setShowSignout(true)}>
        <Ionicons name='log-out-outline' size={28} color={"#FFFFFF"} />
      </Pressable>

      {showSignout && <ModalSignOut signOut={handleSignout} />}
      ...
      <View className='rounded-xl bg-background-light h-[40%] mt-8 overflow-hidden'>
        ...
        <Pressable 
          ...
          className='flex-row h-[33%] bg-background-light px-4 items-center justify-between border-b border-slate-800'>
        ...
        {/* Delete Profile Card */}
        <Pressable className='flex-row h-[34%] bg-background-light px-4 items-center justify-between'>
          <View className='flex-row items-center gap-4'>
...
function ModalSignOut({ signOut }: { signOut: (choice: boolean) => void }) {
  return (
    <View className='absolute bg-gray-900 h-48 w-[85%] z-50 my-36 rounded-2xl items-center justify-center gap-8 self-center border border-slate-700 shadow-2xl'>
      ...
      <View className='flex-row px-6 gap-4'>
        <Pressable
          ...
          className='bg-slate-700 rounded-xl h-12 flex-1 justify-center items-center'>
          <Text className='text-text-primary text-lg'>Cancel</Text>
        </Pressable>
        <Pressable
          onPress={() => signOut(true)}
          className='bg-red-500 rounded-xl h-12 flex-1 justify-center items-center'>
