...
import { CreatePollParams, Nominees, PollCommentsResponse, PollDetailResponse, PollResponse, SearchResult } from "@/types/poll";
...
export async function searchNominees({searchInput, alreadyLoaded}:{searchInput:string, alreadyLoaded:number}){
    let isSignedIn = true
...
export async function createNomineeProfile(
    formData:FormData
){
   let isSignedIn = true
...
export async function createPoll({formData}:{formData:FormData}) {
    ...
    try {
        ...
        data = await response.json();
        console.log("RESPONSE IN POLLCERA:", response)
...
export async function getPollComments({pollId, page}:{pollId:string, page:number}) {
    let isSignedIn = true
...
export async function addComment({pollId, page}:{pollId:string, page:number}) {
    let isSignedIn = true
    let data:PollCommentsResponse;
    try {       
        const cookieStore = await cookies();
...
export async function updatePoll({pollId, bodyParam}:{pollId:string, bodyParam:updatePollParams}) {
    let isSignedIn = true
    let data;
    try {       
        console.log("POLL P[ARAMS",bodyParam)
        const cookieStore = await cookies();
        const token = cookieStore.get("auth_token")?.value;
